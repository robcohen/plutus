...
in rec {
  ...
  plutus-scb = pkgs.recurseIntoAttrs (rec {
    ...
    demo = let 
              ...
              node-port = "8082"; # needs to be the same for all PABs
              scb = "${haskell.packages.plutus-scb.components.exes.plutus-scb}/bin/plutus-scb";
              ...
              process-outboxes = pkgs.writeTextFile {
                  ...
                  while [ 1 ]
                  do
                      ${scb} --config=${config-primary} contracts process-outboxes
                      ${scb} --config=${config-secondary} contracts process-outboxes
                      sleep 3
                  done
                  '';
                  executable = true;
          ...
          in pkgs.stdenv.mkDerivation {
            ...
            installPhase = ''
              ...
              ln ${process-outboxes} process-outboxes.sh
            '';
          }; 
  });
